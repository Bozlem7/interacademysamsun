...
const SPECIALTY_LABEL: Record<string, string> = {
  antrenor: "Antrenör Oturumu",
  diyetisyen: "Diyetisyen Oturumu",
  psikolog: "Psikolog Oturumu",
};
...
export function StaffLoginPage() {
  ...
  const otherSessionOpen = !!session.token && !!session.role && session.role !== "egitmen";
  ...
  async function submit() {
    ...
    setError("");
    try {
      const { token, user, branch } = await staffOrAdminLogin("staff", username, password, activeBranch.code);
      login({
        token,
        role: "egitmen",
  ...
  return (
    <div className="flex min-h-[calc(100vh-64px)] items-center justify-center px-5 py-16">
      <div className="w-full max-w-md">
        ...
        <h2 className="text-2xl font-extrabold text-slate-900 dark:text-white">Eğitmen &amp; Uzman Girişi</h2>
        <p className="mt-2 mb-6 text-sm text-slate-500 dark:text-slate-400">Kullanıcı adınız ad-soyad birleşiminizdir.</p>
        {otherSessionOpen && (
