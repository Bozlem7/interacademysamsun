...
/** Yoklama raporunun içeriği (özet kartlar + tablo) — hem popup içinde hem inline yerleşimde kullanılır. */
export function AttendanceReportContent({ studentId, active }: { studentId: string | null; active: boolean }) {
...
export function AttendanceReportModal({
  ...
  // Portal + z-[10000]: bu diyalog, açıldığı öğrenci detay ekranı hâlâ açık bir Modal'ın
  // (z-[9999], kendi de <body>'e portallanmış) içinden tetikleniyor. Eskiden normal DOM
  // dalında render edildiği için aynı z-index'e sahip olsa bile DOM'da sonradan eklenen
  // (dolayısıyla üstte kalan) Modal portalının ARKASINDA kalıyordu — bkz. Modal.tsx'teki
  // ConfirmDialog için aynı nedenle uygulanmış çözüm.
  return createPortal(
    <div
      className="fixed inset-0 z-[10000] flex items-center justify-center overflow-y-auto bg-black/75 p-4 backdrop-blur-sm sm:p-5"
      onClick={onClose}
    >
      <div
      ...
      >
        ...
        <div className="mb-4 text-xl font-extrabold text-slate-900 dark:text-white">
          Yoklama Raporu{studentName ? ` — ${studentName}` : ""}
        </div>

        <AttendanceReportContent studentId={studentId} active={open} />
      </div>
    </div>,
    document.body
  );
